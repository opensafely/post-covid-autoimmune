...
write.table(count_df,quote=F,row.names=T,col.names=T,"output/not-for-review/study_counts_prepro.txt")

#summary data 
describe_data <- function(data) {
  file_name <- paste0("output/not-for-review/describe_prepro_", deparse(substitute(data)), ".txt")
  sink(file_name)
